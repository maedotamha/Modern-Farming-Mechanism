  ...
  const sendPumpStatusToBluetooth = async (status) => {
    try {
      ...
      const device = await navigator.bluetooth.requestDevice({
        ...
        optionalServices: [0x1101], // Replace with your service UUID
      });
      ...
      const service = await server.getPrimaryService(0x1101); // Replace UUID
      const characteristic = await service.getCharacteristic(0x1101); // Replace UUID
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="bg-yellow-100 text-yellow-800 p-4 rounded-md mb-6">
        ...
        <p className="text-sm">
          You are manually controlling the pump. This action may interfere with the automated irrigation system. Proceed with caution.
        </p>
      </div>
      ...
      <div className="relative h-48 rounded-xl overflow-hidden mb-8">
        ...
        <div className="absolute inset-0 bg-gradient-to-r from-green-900/80 to-transparent flex items-center">
          <div className="p-8">
            ...
            <p className="text-green-50">Manage your farm operations with ease</p>
          </div>
        </div>
      </div>
